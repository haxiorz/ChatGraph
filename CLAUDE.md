...
Graph engine upgrade (Tier 1): ELK.js layout engine replacing dagre (F060), semantic zoom with 3-level detail nodes (F061), multiple layout algorithm picker — hierarchical/tree/force/radial (F062), animated edge streaming indicator with SVG particles (F063), smooth layout transitions via CSS (F064), dynamic node sizing by significance metrics (F065), fisheye focus+context distortion mode (F066), topic-based node clustering with manual assignment and colored regions (F067), edge data flow visualization with token count labels (F068), interactive pannable/zoomable minimap (F069), graph background pattern options — dots/lines/cross/none (F070).

AI capabilities (Tier 2): smart model router with client-side prompt classifier (F078), thinking/reasoning level toggle with per-message Fast/Thinking/Deep modes and thinking content display (F079), file upload & document processing with drag-drop, text/PDF extraction, and multimodal image support (F084), agent task visualization with collapsible tool call sub-nodes in chat and graph (F085), interactive reasoning tree with parsed thinking steps visualization (F086).
