...
export function ContextBar() {
  ...
  if (ratio > 0.8) colorClass = 'bg-red-500'
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 pb-1">
      ...
      <button
        ...
        className="flex w-full items-center gap-2 text-[10px] text-fg-muted group"
      >
              ...
              return (
                <div
                  ...
                  className={`h-full transition-all duration-300 ${
                    isExcluded
                      ? 'bg-red-400/30'
                      : (ROLE_COLORS[entry.node.role] ?? colorClass)
                  } ${i === 0 ? 'rounded-l-full' : ''}`}
      ...
      {expanded && nodeBreakdown.length > 0 && (
        <div className="mt-2 max-h-48 space-y-1 overflow-y-auto rounded-lg border border-border bg-surface p-2">
          {/* Legend */}
          <div className="flex gap-3 pb-1 border-b border-border mb-1">
